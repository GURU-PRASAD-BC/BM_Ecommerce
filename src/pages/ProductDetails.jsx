...
const SingleProductPage = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`https://bm-ecommerce-backend.onrender.com/products/${productID}`);
  ...
  const handleBuyNow = () => {
    addToCart({ id: product.id, image: product.image, title: product.title, price: product.price, quantity_stock: product.quantity_stock, quantity, subTotal });
    navigate('/checkout');
  };
            ...
            isAuthenticated ?
              <div className='buttons'>
                {quantity_stock > 0 &&
                  ...
                  <Button handleClick={handleBuyNow}>
                Buy Now
              </Button>
